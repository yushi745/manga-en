...
**WebFetch必須ルール（新規・リライト共通）**
- 既に知識があっても**必ず一次情報を参照する**。記憶ベースで書くことを禁止する
- 最低でも以下のいずれか1つは参照する：
  - 日本語Wikipedia（基本ファクト確認）
  - ニコニコ大百科・ピクシブ百科事典（ファン視点の詳細）
  - 公式出版社ページ・公式作品サイト（最新の英語版状況・巻数）
  - 英語版Wikipedia（英語圏での評価・anime adaptation情報）
- **Why:** 記憶だけで書くと細部の事実誤認（巻数・キャラ名・出版年・刊行状況）が発生する。Kazoku Gameのように記事自体が存在しない作品を取り違えるリスクもある

**WebFetchソースの検証ルール（重要）**
- WebFetchで取得した情報も**盲目的に信用しない**。Wikipedia・ファン百科事典・まとめサイトは間違っていることがある
- 以下の場合は最低2つのソースで照合する：
  - 巻数・連載状況・英語版ライセンス状況（マーケット情報として更新が遅れる場合がある）
  - キャラクター名の読み（漢字の読みは複数候補があることが多い）
  - 出版社・レーベル（移籍・終了・再出版で変わる）
  - 完結日付・休載期間
- 矛盾を発見したら**WebSearchで追加調査して裏取りする**。複数ソースで一致を確認できない場合のみ、記事に「未確認」と明示するか、その情報自体を記事から省く（誤情報を断定するより不確実性を残す方が良い）
- ユーザーに確認するのは「WebSearchでも判断つかない」かつ「記事の核心に関わる事実」の場合のみ。マイナーな細部は曖昧にして書かない選択をする
- **Why:** WebFetchで取得したWikipediaに誤りがあった場合、それを記事に書くと事実誤認が記事を経由して広まる。一次情報の信頼度を吟味する姿勢が必要
